...
# attach file or folder paths and create several saved conversations.
# يمنح هذا المثال الأخير وكيل الملفات واجهة محادثة مكتبية. يمكن للمستخدم إرفاق
# مسارات ملفات أو مجلدات وإنشاء عدة محادثات محفوظة.
...
class AgentWorker(QThread):
    ...
    def run(self) -> None:
        ...
        try:
            result = Runner.run_sync(file_agent, self.agent_input, session=session)
            self.response_ready.emit(str(result.final_output))
